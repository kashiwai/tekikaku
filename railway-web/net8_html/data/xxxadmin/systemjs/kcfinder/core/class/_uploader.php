...
$result = array();
$error = array();
		if (mb_strlen($message) > 0) $error["message"] = $message;


        $CKfuncNum = isset($this->opener['CKEditor']['funcNum']) ? $this->opener['CKEditor']['funcNum'] : 0;
//		$CKfuncNum = preg_replace('/[^0-9]/', '', (string)$CKfuncNum);
/*
        if (!$CKfuncNum) {
//           $CKfuncNum = '0';
			$result["uploaded"] = 0;
...
$result = "<script type=\"text/javascript\">\n"
		. "window.parent.CKEDITOR.tools.callFunction(" . $CKfuncNum . ", '" . $url . "', '" . $message . "');\n"
		. "</script>";
return $result;
//return "<script type=\"text/javascript\">window.parent.CKEDITOR.tools.callFunction(1, \"$url\", \"\");</script>";
...
/*
		...
		$funcNum = json_encode($CKfuncNum);
		$retUrl = json_encode($url);
		$retMsg = json_encode($message);

return "<script type='text/javascript'>" . "\n"
.    "window.parent.CKEDITOR.tools.callFunction(" . $funcNum . ", " . $retUrl . ", " . $retMsg . ");" . "\n"
. "</script>";
*/
