...
var pslog = bunyan.createLogger({
  name: 'peerjs',
  streams: [
    {
      type: 'rotating-file',
      path: '/var/log/peerjs/peer.log',
      period: '1d',
      count: 14
    }
  ]
...
app._initializeWSS = function(server) {
  ...
  this._wss.on("connection", function(socket, req) {
	...
	//-----2019-02-08 add start--  query.key = apikey_authkey
/*
	var clip = socket._socket.remoteAddress;
	var modkey = query.key;
	var spkey = modkey.split('_');
	if ( spkey.length == 2 ){
		var key = spkey[0];
		var authkey = spkey[1];
	} else {
...
*/
    ...
    if (!id || !token || !key) {
      //-----2019-02-08 log add
  	  pslog.error( 'key:fail ip:'+ip+' querykey:'+query.key );
      socket.send(
        JSON.stringify({
          type: "ERROR",
	...
	//-----2019-02-08 update start--  authkey exist sqlite3
	//-----TEMPORARY: Authentication bypassed for testing
	pslog.info( 'auth:bypassed ip:'+ip+' key:'+key+' token:'+token );
