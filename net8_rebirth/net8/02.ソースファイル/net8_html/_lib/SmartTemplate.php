...
define("TYPE_PC"           , 0);	// PC
define("TYPE_SMART_PHONE"  , 1);	// スマートフォン
define("TYPE_FEATURE_PHONE", 2);	// ガラケー(フィーチャーフォン)
//--- アクセスキャリア 列挙体もどき
define("CARRIER_NAN"       , 0);	// PC他 特定キャリアなし
define("CARRIER_DOCOMO"    , 1);	// DoCoMo
define("CARRIER_AU"        , 2);	// AU
define("CARRIER_SOFTBANK"  , 3);	// Softbank
define("CARRIER_WILLCOM"   , 4);	// Willcom
define("CARRIER_EMOBILE"   , 5);	// EMOBILE
...
class SmartTemplate {
	...
	public function open($p_filename) {
		...
		if ($this->m_enc != ini_get("mbstring.internal_encoding")) {
			// 記述エンコード→PHP処理エンコード
			$this->m_current[$this->m_index]["base"]
				 = mb_convert_encoding($this->m_current[$this->m_index]["base"], ini_get("mbstring.internal_encoding"), $this->m_enc);
		}
