...
try {
    ...
    // デフォルトコーナーデータ
    $corners = [
        ['スロットコーナー', 'SLOT_CORNER'],
        ['パチンココーナー', 'PACHINKO_CORNER'],
        ['新台コーナー', 'NEW_MACHINE_CORNER'],
    ];
    ...
    $stmt = $pdo->prepare("
        INSERT INTO mst_corner (corner_name, corner_roman, del_flg, add_no, add_dt)
        VALUES (:corner_name, :corner_roman, 0, 1, NOW())
    ...
    $corners_list = $pdo->query("
        SELECT corner_no, corner_name, corner_roman
        FROM mst_corner
        WHERE del_flg = 0
        ORDER BY corner_no
