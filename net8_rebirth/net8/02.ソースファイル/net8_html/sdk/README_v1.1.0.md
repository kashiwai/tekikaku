...
- ✅ **ポイント追加機能** - `game.addPoints()` でプレイ中にボーナスポイント付与
- ✅ **リアルタイム状態取得** - `game.getGameState()` で現在のゲーム状態を取得
- ✅ **UIコンポーネント** - 機種選択・ゲーム遷移UIを簡単実装
- ✅ **推奨機種API** - 残高に応じた推奨機種を自動提案
- ✅ **エラーハンドリング強化** - タイムアウト・リトライ機能実装
- ✅ **セキュリティ強化** - postMessage Origin whitelisting実装
...
### game.addPoints(amount, description) 🆕

プレイ中にボーナスポイントを追加します（v1.1.0で追加）。
...
// デイリーボーナス付与
await game.addPoints(1000, 'デイリーボーナス');
```

**パラメータ**:
- `amount` (number, 必須): 追加するポイント数（正の整数）
- `description` (string, オプション): ポイント追加の説明文
...
**エラー**:
- `INSUFFICIENT_BALANCE`: ゲームがユーザーIDなしで開始された場合
- `SESSION_NOT_ACTIVE`: ゲームがアクティブではない場合

**イベント**: `pointsAdded` イベントが発火します

---

### game.getGameState() 🆕

リアルタイムのゲーム状態を取得します（v1.1.0で追加）。

```javascript
const state = game.getGameState();
console.log('クレジット:', state.credit);
console.log('プレイポイント:', state.playpoint);
console.log('抽選ポイント:', state.drawpoint);
console.log('BB回数:', state.bb_count);
console.log('RB回数:', state.rb_count);
console.log('総回転数:', state.total_count);
```

**戻り値**: `GameState`
```javascript
{
    credit: 100,           // 現在のクレジット数
    playpoint: 500,        // プレイポイント
    drawpoint: 200,        // 抽選ポイント
    bb_count: 5,           // BB回数
    rb_count: 3,           // RB回数
    total_count: 150,      // 総回転数
    isPlaying: true,       // プレイ中かどうか
    lastUpdate: 1700000000000,  // 最終更新タイムスタンプ
    sessionId: 'gs_xxxxx', // セッションID
    machineNo: 1,          // マシン番号
    isStarted: true,       // ゲーム開始済みか
...
const updateInterval = setInterval(() => {
    ...
    document.getElementById('bb-count').textContent = state.bb_count;
    document.getElementById('rb-count').textContent = state.rb_count;
...
`net8-ui-components.js` を使用して、機種選択やゲーム遷移のUIを簡単に実装できます。

#### UIコンポーネントの読み込み

```html
<script src="https://mgg-webservice-production.up.railway.app/sdk/net8-ui-components.js"></script>
```
...
const response = await fetch(
    `${Net8.apiUrl}/api/v1/recommended_models.php?balance=5000&limit=5`,
    {
        headers: {
            'Authorization': `Bearer ${Net8.token}`
        }
    }
);
...
// 機種選択UIを表示
ui.showMachineSelector(data.models, async (selectedModel) => {
    // 選択された機種でゲーム開始
    const game = Net8.createGame({
...
```javascript
const balance = 5000; // ユーザーの現在残高

const response = await fetch(
    `${Net8.apiUrl}/api/v1/recommended_models.php?balance=${balance}&limit=3`,
    {
        headers: {
            'Authorization': `Bearer ${Net8.token}`
        }
...
**パラメータ**:
- `balance` (number, 必須): ユーザーの残高
- `limit` (number, オプション): 取得する機種数（1-10、デフォルト: 3）
...
- **[クイックスタートガイド](./QUICKSTART_GUIDE.md)** - 5分で始める最速統合ガイド
- **[トラブルシューティング](./TROUBLESHOOTING.md)** - よくある問題と解決方法
- **[利用規約](./TERMS_OF_SERVICE.md)** - SDK利用規約（必ずお読みください）

### 🔧 技術者向けドキュメント

- **[API仕様書](./API_SPECIFICATION.md)** - 全エンドポイントの詳細仕様
- **[完全なAPIリファレンス](./README_v1.1.0.md)** - このドキュメント（全機能網羅）
...
- ✅ `game.on('pointsAdded')` イベント追加
- ✅ `game.on('score')` イベント追加（リアルタイムスコア更新）
- ✅ `game.on('bonus')` イベント追加（BB/RB突入通知）
- ✅ `game.stop()` メソッド追加
- ✅ `game.addPoints()` メソッド追加（プレイ中ポイント付与）
- ✅ `game.getGameState()` メソッド追加（リアルタイム状態取得）
- ✅ 自動ポイント消費・払い出し機能
- ✅ ユーザー自動登録機能
- ✅ UIコンポーネント（`net8-ui-components.js`）
- ✅ 推奨機種API（`/api/v1/recommended_models.php`）
- ✅ ポイント追加API（`/api/v1/add_points.php`）

### セキュリティ強化
- ✅ postMessage Origin whitelisting実装
- ✅ タイムアウト機能（30秒）
- ✅ リトライ機能（3回、Exponential backoff）
- ✅ トランザクション管理強化（ACID準拠）
