<?php
/*
 * play_test_noauth.php
 *
 * テスト用プレイページ（認証なし）
 *
 * 開発・デバッグ専用：ログイン認証をスキップしてゲーム画面を表示
 * test@example.com のユーザーとして自動的にログインします
 *
 * URL: https://mgg-webservice-production.up.railway.app/data/play_test_noauth.php?NO=1
 */
...
function DispTop($template) {
	...
	//【テストモード】認証なし - test@example.com のユーザーとして自動ログイン
	if ( !isset($template->Session->UserInfo) ) {
		...
		if (empty($userRow["member_no"])) {
			...
			echo "<p>test@example.com のユーザーが見つかりません。</p>";
			echo "<p>データベースを確認してください。</p>";
			return;
		}
	...
	//【テストモード】テスターフラグを強制的に1に設定
	$testerRow = array("member_no" => $template->Session->UserInfo["member_no"], "tester_flg" => "1");

	// 【テストモード】営業時間チェックをスキップ（tester_flg=1なので自動的にスキップされる）
