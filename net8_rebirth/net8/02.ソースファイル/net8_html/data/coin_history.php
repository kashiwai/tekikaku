 ...
 * @since    2025/01/13 coin_history.phpに名称変更
 */
...
function DispList($template) {
	...
	if ( !array_key_exists( $_GET["TYPE"], $GLOBALS["pointHistoryProcessCode"])) $_GET["TYPE"] = "";
	if( mb_strlen($_GET["TYPE"]) > 0){
		if ( !preg_match("/^[0-9]+$/", $_GET["TYPE"])) {
			$_leftjoin = "inner join his_point hp on hp.member_no = men.member_no";
		}else{
			$_leftjoin = 'inner join his_point hp on hp.member_no = men.member_no and hp.proc_cd = "'. $_GET["TYPE"] . '"';
		}
	}else{
		$_leftjoin = "inner join his_point hp on hp.member_no = men.member_no";
	}
	...
	$count_sql = $sqls->setAutoConvert( [$template->DB,"conv_sql"] )
		->select()
			->field("men.point, count(*) as cnt, count(hp.member_no) as hcnt")
			->from("mst_member men")
			->from( $_leftjoin)
			->where()
	...
	$row_sql = $sqls->resetField()
			->field("men.point, hp.proc_dt, hp.type, hp.proc_cd, hp.before_point, hp.point as mpoint, hp.after_point, hp.limit_dt")
	...
	// 項目種別等
	$template->assign("POINT"      , number_format( $mypoint), true);
	$template->assign("SEL_TYPE"   , makeOptionArray($GLOBALS["pointHistoryProcessCode"], $_GET["TYPE"], true, SELECT_VALUE_NONE));
	// リスト
	if( $allrows > 0){
		// ページング
		...
		if( $allrows > 0){
			while ($row = $rs->fetch(PDO::FETCH_ASSOC)) {
				...
				$template->assign("AFTER_POINT"       , number_format( $row["after_point"]), true);
				$template->assign("TYPE_LABEL"        , $GLOBALS["pointHistoryProcessCode"][ $row['proc_cd']], true);
				$template->assign("PROC_DT"           , format_datetime($row["proc_dt"]), true);
				$template->assign("LIMIT_DT"          , format_datetime($row["limit_dt"]), true);
				$template->if_enable("LIMIT_DT"       , mb_strlen($row["limit_dt"]) > 0, " - ");
				$template->loop_next();
			}
		}
