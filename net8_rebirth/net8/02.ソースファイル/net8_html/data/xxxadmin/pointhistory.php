...
function DispList($template, $message = "") {
	...
	$csql = $sqls->setAutoConvert( [$template->DB,"conv_sql"] )
			...
			->where()
				...
				->and(true, "mem.agent_flg = ", $_GET["S_AGENT"], FD_NUM)	// 2021/01 [ADD]
			->createSql("\n");
	...
	// 2021/01 [ADD Start] 合計追加
	$tsql = $sqls->resetField()
			->field("SUM(IF(his.type = " .  $template->DB->conv_sql(2, FD_NUM) . ", his.point, 0)) as sub_point")
			->field("SUM(IF(his.type = " .  $template->DB->conv_sql(1, FD_NUM) . ", his.point, 0)) as add_point")
		->createSql("\n");
	...
	$rsql = $sqls
			...
			->field("his.key_no, his.limit_dt, his.after_point")
			->field("IF(his.type = " .  $template->DB->conv_sql(2, FD_NUM) . ", his.point, 0) as sub_point")
			->field("IF(his.type = " .  $template->DB->conv_sql(1, FD_NUM) . ", his.point, 0) as add_point")
			->field("mem.nickname, mem.black_flg, mem.state, mem.tester_flg")
			->field("mem.agent_flg")	// 2021/01 [ADD]
	...
	// 2021/01 [ADD Start] エージェント検索、合計追加
	$template->if_enable("POINTHISTORY_DISP_TOTAL", POINTHISTORY_DISP_TOTAL);	// 合計表示
	$template->if_enable("POINTHISTORY_SEL_AGENT" , POINTHISTORY_SEL_AGENT);	// エージェント検索
	// 2021/01 [ADD End] エージェント検索、合計追加

	// 検索
	$template->assign("S_OPEN"        , $_search, true);
	$template->assign("S_MEMBER_NO"   , $_GET["S_MEMBER_NO"], true);
	$template->assign("SEL_PROC_CD"   , makeOptionArray($GLOBALS["pointHistoryProcessCode"], $_GET["S_PROC_CD"], true));
	$template->assign("S_PROC_DT_FROM", $_GET["S_PROC_DT_FROM"], true);
	$template->assign("S_PROC_DT_TO"  , $_GET["S_PROC_DT_TO"], true);
	// 2021/01 [ADD Start] エージェント検索
	...
	// 2021/01 [ADD Start] 合計追加
	$template->assign("DISP_TOTAL_SUB_POINT", number_formatEx($totals["sub_point"]), true);
	$template->assign("DISP_TOTAL_ADD_POINT", number_formatEx($totals["add_point"]), true);
	// 2021/01 [ADD End] 合計追加


	// リスト処理
	$template->loop_start("LIST");
	while ($row = $rs->fetch(MDB2_FETCHMODE_ASSOC)) {
		$isPlay = ($row["proc_cd"] == "51" && mb_strlen($row["key_no"]) > 0);
		...
		$template->if_enable("IS_AGENT"   , $row["agent_flg"] == 1);	// 2021/01 [ADD]
		...
		$template->assign("PROC_NAME"    , $template->getArrayValue($GLOBALS["pointHistoryProcessCode"], $row["proc_cd"]), true);
		...
		$template->assign("LIMIT_DT"     , format_datetime($row["limit_dt"]), true);
		$template->assign("AFTER_POINT"  , number_formatEx($row["after_point"]), true);
...
function ProcOutput($template) {
	...
	$sql = (new SqlString())
			...
			->select()
				...
				->field("IF(his.type = " .  $template->DB->conv_sql(2, FD_NUM) . ", his.point, 0) as sub_point")
				->field("IF(his.type = " .  $template->DB->conv_sql(1, FD_NUM) . ", his.point, 0) as add_point")
				->field("his.limit_dt, his.after_point")
			->from("his_point his")
			->join( "inner", "mst_member mem", "his.member_no = mem.member_no")
			->where()
				->and(true, "his.member_no = ", $_GET["S_MEMBER_NO"], FD_NUM)
	...
	while ($row = $outRs->fetch(MDB2_FETCHMODE_ASSOC)) {
		...
		if (POINTHISTORY_SEL_AGENT && $row["agent_flg"] == "1") $addState[] = GIFT_AGENT_DISPNAME;	// 2021/01 [ADD]
		if (count($addState) > 0) {
			$row["nickname"] .= "（" . implode("、", $addState) . "）";
		}
		$row["state"] = $template->getArrayValue($GLOBALS["MemberStatus"], $row["state"]);
		$row["proc_cd"] = $template->getArrayValue($GLOBALS["pointHistoryProcessCode"], $row["proc_cd"]);
		$row["limit_dt"] = format_datetime($row["limit_dt"]);
		// 不要項目削除
		unset($row["tester_flg"]);
		unset($row["black_flg"]);
		unset($row["agent_flg"]);	// 2021/01 [ADD]
		// 出力領域に設定
		$row = str_replace('"', '""', $row);
	...
	header("Content-Disposition: attachment; filename=PlayPoint_" . $currentDatetime . ".csv");
