 ...
 * @language  JavaScript
 * @version   1.0
 * @since     2020/11/11 ver 1.0          村上俊行 初版作成
 *            2022/05/11 ver 1.0.7-s5     村上俊行 1日1回の再起動用reload処理を削除
 *            2023/04/14 ver 1.0.7-s6     村上俊行 精算行動終了コードを設定
 *            2023/09/25 ver 1.0.7-s7     村上俊行 readyコードが受信できなかった場合に強制精算にならないように修正

 * @desc
 */
 	//サーババージョンを記述
	var serverVersion = '1.0.7-s7';
	...
	var keyMapping = {
		...
		'bas':   { 'msec' : 1000, 'oldtime': null },			//auto on/off
...
//		'bs3':   { 'msec' : 1000, 'oldtime': null },			//REAL STOP3
		'bsoc':  { 'msec' :  500, 'oldtime': null },			//CHANCE(set)
	...
	function resetGame(){
		var game = {
			...
			//動作設定
			'min_credit'    : 3,
			'conv_point'    : <?php echo $CONVPLAYPOINT; ?>,  // 変換ポイント
			'conv_credit'   : <?php echo $CONVCREDIT; ?>,     // 変換クレジット
			//2020-09-23 追加情報
			'renchan_count' : 0,
			'maxrenchan_count' : 0,
			'tenjo_count'   : 0,
...
//				console.log("python event ", event.data);
				...
				if ( event.data == '@startUSBIO' ){
					...
					//DigiCounterの接続が切れた。
					setCameraStatus("end");
				} else if ( event.data == '@delaystartUSBIO' ){
					// 2023-04-20 ここでメンテナンス中の倍が開始させない
					if (!$('#mentebutton').hasClass('deactive')) {
						//2021-05-07 精算後再起動の10秒を超えるように開始設定をかける
						setTimeout(function(){
							setCameraStatus("start");
						}, 15000);
					}
				...
				if ( _sconnect ){
					...
					//放置時間計測用
					if (_tag.substr(0,7) == 'Signal_' || _tag == 'STG' || _tag.substr(0,5) == 'HESO_'){
						//シグナルのみ計測
						lastTimestamp = (new Date()).getTime();
						//leave reset
						$('#nowleavetime').text('');
					...
					if( _tag == 'Signal_Game+1' ){
						...
						game.credit -= cr;
						game.in_credit += cr;
						game.mc_in_credit += cr;
						//2021-06-01 user別追加
						game.user_in_credit += cr;
					} else if ( _tag.substr(0,4) == 'CRO_' ){				//CreditOut
						var crcd = _tag.split('_');
						var cr = parseInt(crcd[1]);
						game.credit += cr;
						game.out_credit += cr;
						game.mc_out_credit += cr;
						saveCredit += cr;
						addCredit += cr;
						//2021-06-01 user別追加
						game.user_out_credit += cr;
						//別電文を追加
						_sconnect.send( _sendStr('UCR_'+game.user_out_credit, '') );
						...
						*/
						...
						if ( !activeBB ){
							game.rb_count++;
							game.user_RB++;
							//先にRB処理された時に BB のログを送らないように名称を初期化
							bbsendTag = null;
							...
							playLog('rb')								//ログ記録
							.then(function(){
								keysocket.send('@COMPLETE-RB-LOG');
							},function(data){
						...
						if ( !activeBB ){
							...
							game.bb_count++;
							game.user_BB++;
							if ( game.past_max_bb < game.bb_count ) game.past_max_bb = game.bb_count;
							$('#bb_count').text(game.bb_count);
							$('#bb_on').prop('checked', true);
						}
						$.cookie('bb', 'on', { expires: 365 });
						// 前回シグナルが RB(Signal_2)ならplayLogの処理を遅延させる
						var logdelay = 10;
						if ( bbsendTag == null ) {
							logdelay = 2000;
							keysocket.send('@DELAY-BB-LOG');
						}
						setTimeout(function(){
							playLog(bbsendTag)								//ログ記録
							.then(function(){
								keysocket.send('@COMPLETE-BB-LOG');
								bbsendTag = 'bb'
							},function(data){
...
//						console.log( 'dc:'+_t);
						...
						//basがstart以外ならendコードに修正
						if ( _t == 'bsa' && _msg != 'autostart' ){
							_t == 'bse';
						}
...
//						if ( _t == 'bse' ){
		...
		function checkReadyGame( dc, tm=1000 ) {
			setTimeout(function(){
				...
				_sconnect.send( _sendStr('Adc',  game.day_count) );
				...
				_sconnect.send( _sendStr('MDD_'+game.past_max_credit, '') );			//2020-12-10追加 
				_sconnect.send( _sendStr('MCNT_'+game.max_continuous, '') );			//2020-12-10追加 
				_sconnect.send( _sendStr('MBB_'+game.past_max_bb, '') );				//2020-12-10追加 

				_sconnect.send( _sendStr('UCR_'+game.user_out_credit, '') );			//2021-06-01追加 
				
				console.log( 'MBB_'+game.past_max_bb );
