 ...
 * @since	2017/04/12 日付関連改修 岡本 静子 日付をアメリカ式(mm/dd/yyyy)に変更
 * @info	
 */
...
//	if (!ereg("^([0-9]+)[$sep_format]([0-9]+)[$sep_format]([0-9]+)$", $p_target, $m)) return "";
	...
	if ($p_jp) {
		...
		*/
		/* 2017/04/12 Upd by S.Okamoto 表記をアメリカ式(mm/dd/yyyy)に変更
		$ret = str_pad($m[2], 2, "0", STR_PAD_LEFT);
		if ($p_extday) $ret .= "/" . str_pad($m[3], 2, "0", STR_PAD_LEFT);
		if ($p_extyear) $ret = $m[1] . "/" . $ret;
		*/
		$format = "m";
		if ($p_extday) $format .= "/d";
		if ($p_extyear) $format .= "/Y";
		$ret = date($format, strtotime($p_target));
	}
	return $ret;
}
/**
...
function format_time($p_target, $p_jp = false, $p_extsec = false) {
	...
	if ($p_jp) {
		...
		*/
		...
		/* 24時間表記で統一となったのでコメントアウト
		// 2017/04/12 Upd by S.Okamoto 表記をアメリカ式(00:00:00 am)に変更
		$format = "h:i";
		if ($p_extsec) $format .= ":s";
		$format .= " a";
		$ret_time = date($format, strtotime($p_target));
		*/
	}
 ...
 * @info
 */
function makeOptionArray($p_ary_item, $p_def, $p_nullvalue = true, $p_nulldisp = "", $p_addcode = false, $p_dispadd = "", $p_entity = true) {
...
function makeRadioArray($p_ary_item, $p_name, $p_def = "", $p_add = "", $p_br = false, $p_entity = true) {
	...
	foreach ($p_ary_item as $key=>$value) {
		...
		//--- 2011/06/08 S.Okamoto htmlspecialchars対応
		//$ret .= ">" . $value . "\n";
		$ret .= ">" . (($p_entity) ? htmlspecialchars($value) : $value) . "</label>" . "\n";
		$idx += 1;
	}
...
function makeCheckBoxArray($p_ary_item, $p_name, $p_def = "", $p_aryflg = 0, $p_header = "", $p_footer = "", $p_between = "", $p_entity = true, $p_line_break = 0) {
	...
	foreach ($p_ary_item as $key => $value) {
		...
		if (is_array($p_def)) {
			...
			//--- 2011/06/08 S.Okamoto htmlspecialchars対応
			//$ret .= ">" . $p_between . $value;
			$ret .= ">" . $p_between . (($p_entity) ? htmlspecialchars($value) : $value);
		} else {
			if ((string)$p_def == (string)$key) $ret .= " checked";
			//--- 2011/06/08 S.Okamoto htmlspecialchars対応
			//$ret .= ">" . $p_between . $value;
