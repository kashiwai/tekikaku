...
var _savestream;					//Video確認用
	...
	var koreaMode = true;						//★ EMBED専用: 韓国統合モード（常にtrue）
	var autoModePrep = false;					//韓国用: AUTO待機状態（STARTで開始）

	// ★ EMBED専用: koreaMode は最初から true（すべて韓国リクエスト）
	console.log('✅ [EMBED-INIT] Korea mode ALWAYS ENABLED for /data/play_embed/');
	console.log('🔍 [EMBED-INIT] sessionId:', typeof sessionId !== 'undefined' ? sessionId : 'UNDEFINED (外部から渡されるべき)');
	...
	// ★ EMBED専用: シンプル化されたcallback送信（koreaMode は常に true）
	function sendBetCallback(betAmount, creditBefore, creditAfter) {
		...
		// EMBED専用: sessionId の存在のみ確認（koreaMode は常に true）
		if (typeof sessionId === 'undefined' || !sessionId) {
			console.error('❌ [BET-CALLBACK] SKIPPED! sessionId is missing:', sessionId);
			return;
		}

		console.log('✅ [BET-CALLBACK] Sending to API... (koreaMode: always true)');
	...
	function sendWinCallback(winAmount, creditBefore, creditAfter) {
		...
		// EMBED専用: sessionId の存在のみ確認（koreaMode は常に true）
		if (typeof sessionId === 'undefined' || !sessionId) {
			console.error('❌ [WIN-CALLBACK] SKIPPED! sessionId is missing:', sessionId);
			return;
		}

		console.log('✅ [WIN-CALLBACK] Sending to API... (koreaMode: always true)');
	...
	function sendPointConvertedCallback(creditConverted, pointsReceived, conversionRate) {
		console.log('🎰 [CONVERT-CALLBACK] Called with:', {
		...
		// EMBED専用: sessionId の存在のみ確認（koreaMode は常に true）
		if (typeof sessionId === 'undefined' || !sessionId) {
			console.error('❌ [CONVERT-CALLBACK] SKIPPED! sessionId is missing:', sessionId);
			return;
		}

		console.log('✅ [CONVERT-CALLBACK] Sending to API... (koreaMode: always true)');
...
//				_sconnect.send(_sendStr( 'Lng', languageMode ));
				...
				// ★ EMBED専用: クライアント側のplaypointをカメラに同期
				// koreaMode は最初から true なので、Spt コマンドのみ送信
				if (game.playpoint > 0 && _sconnect && _sconnect.open) {
					console.log('💰 [EMBED] Syncing playpoint to camera:', game.playpoint, '(koreaMode: always true)');
		...
		.then(function(ret){
			...
			if ( $('#autoplay_credit').hasClass('auto-on') && !autoStopSignal ){
				if ( autoFirstEventFlg ){
					...
					console.log('🎮 [autoPlay] First auto event - sending bas after delay');
					setTimeout(function(){
				...
				console.log('⚠️ [autoPlay] AUTO button not in auto-on state or autoStopSignal');
				autoPlay_Off();
			}
		},function(){
			//credit変換できなかった時
