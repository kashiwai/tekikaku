...
function putJson($DB) {
	...
	$ary = array();
	$skp = array();
	foreach ( $GLOBALS["webRTC_camera_Files"] as $filename ){
		$path = $GLOBALS["webRTC_lastupdate_path"].$filename;
		if ( file_exists($path) ){
			$rec["path"]     = $filename;
			$rec["datetime"] = date("Y-m-d H:i:s", filemtime($path) );
			
			//除外リストチェック
			if ( array_key_exists( $machine_no, $GLOBALS["webRTC_Exclusion_List"] ) ){
				//対象machine_noで除外リストにファイル名が存在していたらパッチリストに登録しない
				if ( in_array( $filename, $GLOBALS["webRTC_Exclusion_List"][$machine_no] ) ){
					array_push( $skp, $rec );
					continue;
				}
			}
