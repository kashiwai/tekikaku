	...
	class Trans
	{
		...
		*/
		function __construct ($odata)
		{
			if (empty($odata)) {
				...
				if (defined('GASH_BASE64_LOG')){
					...
					fputs( $fp, date("Y/m/d H:i:s") . " recv " . $odata . "\n");
					fputs( $fp, date("Y/m/d H:i:s") . " recv " . $this->data . "\n");
					fclose($fp);
				}
		...
		public function GetSendData()
		{
			...
			if (defined('GASH_BASE64_LOG')){
				$fp = fopen(GASH_BASE64_LOG, "a");
				fputs( $fp, date("Y/m/d H:i:s") . " send " . base64_encode( $this->GenerateXmlDoc() ) . "\n");
				fputs( $fp, date("Y/m/d H:i:s") . " send " . $this->GenerateXmlDoc() . "\n");
				fclose($fp);
			}
