...
Socket.prototype.send = function(data) {
  ...
  console.log(data);
  // 2020-03-04 raspberry pi 4 用にcodecを変更する
  // removeCodecListに登録されているcodecをSDPから削除する
  if ( data.payload.sdp ){
	  if ( data.payload.sdp.sdp ){
		  var repsdp = data.payload.sdp.sdp;
		  removeCodecList.forEach( function(codecName){
			  repsdp = removeCodec(repsdp, codecName);
		  });
		  data.payload.sdp.sdp = repsdp;
		  console.log(data);
	  }
  }
...
var util = {
  ...
  supports: (function() {
    ...
    var pc, dc;
    try {
      pc = new window.RTCPeerConnection(defaultConfig, {
        // 2021-08-17 deleted
...
Unpacker.prototype.read = function(length){
  ...
  if (j + length <= this.length) {
    return this.dataView.subarray(j, j + length);
  	...
  	return this.dataView.subarray(j, j + length - 1);
