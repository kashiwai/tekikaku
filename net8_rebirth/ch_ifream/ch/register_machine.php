...
try {
    ...
    foreach ($machines as $machine) {
        ...
        if ($exists) {
            echo "<p>⚠️ {$machine['machine_cd']} は既に登録済み（台No: {$exists['machine_no']}）</p>";
            $skipped_count++;
            continue;
        }
        ...
        echo "<p>✅ {$machine['machine_cd']} 登録完了（台No: {$machine_no}、カメラ: {$machine['camera_no']}、シグナリング: {$machine['signaling_id']}）</p>";
        $registered_count++;
    }
    ...
    echo "<hr>";
    echo "<h2>📋 登録された実機一覧</h2>";
    $machines_list = $pdo->query("
        SELECT m.machine_no, m.machine_cd, m.camera_no, m.signaling_id, m.release_date
        FROM dat_machine m
    ...
    echo "<tr><th>台No</th><th>台CD</th><th>カメラNo</th><th>シグナリングID</th><th>公開日</th></tr>";
