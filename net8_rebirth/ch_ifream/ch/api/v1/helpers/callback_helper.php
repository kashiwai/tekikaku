...
function sendRealtimeCallback($callbackUrl, $callbackSecret, $eventType, $data, $maxRetries = 3) {
    // HTTPS必須チェック（ローカルテスト用にlocalhostは許可）
    ...
    if (!$isHttps && !$isLocalhost) {
        error_log("❌ Callback URL must be HTTPS (or localhost for testing): {$callbackUrl}");
        return [
            'success' => false,
            'error' => 'CALLBACK_URL_NOT_HTTPS',
            'message' => 'Callback URL must use HTTPS (localhost is allowed for testing)'
        ];
    }

    if ($isLocalhost) {
        error_log("⚠️ [LOCAL TEST MODE] Allowing HTTP callback to localhost: {$callbackUrl}");
    }
